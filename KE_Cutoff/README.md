# Kinetic Energy Cutoff Optimization
    ...
    This is the naming convention I developed my workflow for. The naming convention is important if you decide to follow my script. The make files, job files, extraction files, and eventually, 
    the plotting scripts parse files with that specific naming convention.

    The 'KE_DA' folder performs the bulk of the data analysis for the KEC optimization. Within it, you will find 'plot_ke_vs_results.py' and 'min_KE_cut.py'. The plotting script parses through data (txt)
    files generated from 'extract_KE.py' with the aforementioned naming convention. Running it should produce two plots: "Total Energy vs. KE Cutoff" & "Run Time vs. KE Cutoff". They will also write
    to .png files.
